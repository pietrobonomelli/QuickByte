...
public class DettaglioOrdineDAOTest {
	...
	private final Ordine testOrdine = new Ordine(1, StatoOrdine.PENDENTE, 20.00, 
			dataOra, "Via Test 123", testCliente.getEmail(), null, 1);
	...
	private void insertTestData() throws SQLException {
		...
		try (Connection conn = DatabaseConnection.connect();
			...
			stmt.setString(2, testOrdine.getStato().name()); // Converti StatoOrdine in stringa
			stmt.setDouble(3, testOrdine.getCosto());
			stmt.setTimestamp(4, testOrdine.getDataOraOrdine()); // Usa setTimestamp per Timestamp
